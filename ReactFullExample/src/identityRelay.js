...
iframe.onload = () => {
    ...
    win.postMessage(postMsg, settings.serverUrl);
...
window.onmessage = function (e) {
    if (e.origin !== settings.serverUrl) {
        return;
    }
    ...
    if (e.data.type === "USER_INFO") {
        ...
        const keys = Object.keys(localStorage).filter((key) =>
            key.startsWith(settings.cognitoLocalStorageKey),
        );
...
function getRelatedEntries() {
    ...
    return localStorageArray.filter((item) => item[0].startsWith(settings.cognitoLocalStorageKey));
}
